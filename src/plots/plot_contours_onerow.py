...
sc = 6.
fs = 20 
fig=figure(facecolor='none',figsize=[sc*len(files)*4./3.,sc])
hold(True)
for iax in arange(len(files)):
    ...
    ax=subplot(1,len(files),iax+1)
    contourf(x/AU,a,log10(S),arange(mx-10,mx+1))
    xscale('log')
    yscale('log')
    L1,=loglog(x/AU,a_fr,mycol,linestyle='--')
...
#
# pack the axes together again to make sure all labels are on the plot
#
gcf().tight_layout()
#
# save the figure
